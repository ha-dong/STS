    ...
    <div class="container">
        ...
        <form id="storyForm" enctype="multipart/form-data">
            ...
            <div class="story-info">
                ...
                <input type="number" id="initialMoney" name="initialMoney" min="0" max="50" value="<c:choose><c:when test='${editMode}'><c:out value='${story.initialMoney}'/></c:when><c:otherwise>0</c:otherwise></c:choose>" required>

                <label for="initialHP">초기 체력 설정 (10~100):</label>
                <input type="number" id="initialHP" name="initialHP" min="10" max="100" value="<c:choose><c:when test='${editMode}'><c:out value='${story.initialHP}'/></c:when><c:otherwise>100</c:otherwise></c:choose>" required>

                <!-- 엔딩 코드 선택 -->
                <label for="endCode">엔딩 코드:</label>
                <select id="endCode" name="endCode">
                    <option value="">선택하세요</option>
                    <option value="CE-01" <c:if test="${editMode and 'CE-01' == story.endCode}">selected</c:if>>해피엔딩</option>
                    <option value="CE-02" <c:if test="${editMode and 'CE-02' == story.endCode}">selected</c:if>>새드엔딩</option>
                    <option value="CE-03" <c:if test="${editMode and 'CE-03' == story.endCode}">selected</c:if>>열린결말</option>
                </select>
            </div>
                            ...
                            <div class="choices" id="choices_${scene.tempSceneNum}">
                                <c:forEach var="choice" items="${scene.choices}">
                                    <div class="choice" data-choice-num="${choice.choiceNum}">
										...
										<!--
                                        <label>돈 변화량:</label>
                                        <input type="number" name="choiceMoney_scene_${scene.tempSceneNum}_choice_${choice.choiceNum}" value="${choice.money}">

                                        <label>체력 변화량:</label>
                                        <input type="number" name="choiceHP_scene_${scene.tempSceneNum}_choice_${choice.choiceNum}" value="${choice.hp}">
										-->
